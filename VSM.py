...
def Process_files(name_file,list_articles,kind):
#  Global function for reading and process the files.
   if kind=="doc":
      w="*TEXT"
   else:
      w="*FIND"
   lines = [line.rstrip('\n') for line in open(name_file)]
   list_articles = [list(y) for x, y in itertools.groupby(lines, lambda z: z.startswith(w)) if not x]
   for i in range(0,len(list_articles)):
      list_articles[i] = " ".join(list_articles[i])
      list_articles[i] = re.sub(r'[^a-zA-Z\s]+', '', list_articles[i])
...
#    terms present in the document as a string.
        self.articles=Process_files(self.name_file_doc,self.articles,kind)
...
#    terms present in the query as a string.
        articles=[]
        articles=Process_files(self.name_file_q,articles,kind)
